describe('Cashier Login Flow', () => {
  beforeEach(() => {
    cy.visit('/auth/cashier/login')
  })

  it('displays the login form', () => {
    cy.get('h2').contains('Cashier Login')
  ...
  it('redirects to cashier POS with valid credentials', () => {
    // In a real test, you would use valid test credentials
    // For now, we'll just test the UI flow
    cy.get('input[name="email"]').type('cashier@example.com')
