...
export default function Settings() {
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'receipts' | 'reset'>('general');
  ...
  const handleResetData = async () => {
    if (resetPassword !== 'Tripz0219!!!') {
      alert('Incorrect security password.');
      return;
    }

    if (!confirm('CRITICAL: This will delete ALL transactions, ALL cashiers, and reset ALL product stocks to 0. This cannot be undone. Proceed?')) {
      return;
    }

    setResetLoading(true);
    try {
      const { success, error } = await supabaseDB.resetApplicationData();
      if (!success) throw new Error(error);
      alert('System reset successfully. All sales and cashier data have been cleared.');
      setResetPassword('');
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-1/4 space-y-2">
          ...
          <button
            ...
            className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-200 ${activeTab === 'reset'
              ? 'bg-red-600 text-white shadow-md'
              : 'hover:bg-red-50 dark:hover:bg-red-900/10 text-red-600'
              }`}
          >
            <div className="flex items-center gap-3">
            ...
            {activeTab === 'reset' && <ChevronRight className="h-4 w-4" />}
          </button>
            ...
            <form className="space-y-6">
              {activeTab === 'general' ? (
                ...
                <Card className="border-red-100 dark:border-red-900/20 shadow-sm overflow-hidden">
                  <div className="p-6 border-b bg-red-50/50 dark:bg-red-900/10">
                    <h3 className="text-xl font-bold text-red-700 dark:text-red-400 flex items-center gap-2">
                      <AlertTriangle className="h-5 w-5" />
                      Critical Data Operations
                    </h3>
                    <p className="text-sm text-red-600/80 dark:text-red-400/80 font-medium">Reset system records back to zero.</p>
                  </div>
                  <CardContent className="p-8 space-y-8">
                    <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800 p-6 rounded-2xl flex items-start gap-4">
                      <div className="h-10 w-10 flex-shrink-0 bg-red-100 dark:bg-red-800 rounded-full flex items-center justify-center text-red-600 dark:text-red-300">
                        <Trash2 className="h-5 w-5" />
                      </div>
                      <div className="space-y-2">
                        <h4 className="font-bold text-red-900 dark:text-red-100">Factory Reset Simulation</h4>
                        <p className="text-sm text-red-800 dark:text-red-200 leading-relaxed">
                          This operation will clear all sales history (transactions), remove all registered cashiers, and reset the stock count of all products to 0.
                        </p>
                        <ul className="text-xs text-red-700 dark:text-red-300 list-disc list-inside mt-2 font-black uppercase tracking-tight">
                          <li>Clears Transactions & items</li>
                          <li>Deletes All Cashier accounts</li>
                          <li>Resets Product Stocks to zero</li>
                        </ul>
                      </div>
                    </div>
                    ...
                    {!showResetForm ? (
                      <Button
                        variant="destructive"
                        size="lg"
                        className="w-full h-14 rounded-2xl text-lg font-black uppercase tracking-widest flex items-center gap-3 shadow-xl shadow-red-500/20 border-b-4 border-red-800"
                      ...
                      <div className="space-y-6 animate-in slide-in-from-bottom duration-300">
                        <div className="space-y-3">
                          ...
                          <Input
                            ...
                            className="h-14 rounded-2xl text-center text-xl font-black border-red-200 focus:ring-red-500 bg-white shadow-inner"
                            value={resetPassword}
                        ...
                        <div className="flex gap-4">
                          ...
                          <Button
                            variant="destructive"
                            className="flex-[2] h-14 rounded-2xl font-black uppercase text-lg shadow-xl shadow-red-500/20"
                            onClick={handleResetData}
                            disabled={resetLoading}
                          >
                            {resetLoading ? 'Resetting...' : 'CONFIRM ERASE'}
                          </Button>
                        </div>
                      </div>
                    )}
